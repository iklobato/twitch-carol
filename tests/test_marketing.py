...
extensionless URLs), in the reader's language (en default, pt for a pt browser)."""
...
def test_howto_defaults_to_english(api_client) -> None:
...
def test_browser_in_a_third_language_gets_english(api_client) -> None:
    # The sales page follows the app: anything that is not Portuguese reads the
    # product in English, instead of landing on a page it cannot read.
    resp = api_client.get("/howto", headers={"Accept-Language": "es-ES,es;q=0.9"})
...
def test_lang_query_overrides_and_sets_cookie(api_client) -> None:
    ...
    resp = api_client.get("/howto?lang=pt", headers={"Accept-Language": "en-US,en;q=0.9"})
