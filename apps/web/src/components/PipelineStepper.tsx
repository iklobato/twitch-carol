...
// Message keys rather than text: the labels are resolved at render time, after
// the screen language is known.
const STEPS: { label: MessageKey; starts?: MessageKey; ends?: MessageKey }[] = [
  {
    label: "pipeline.capture",
    starts: "pipeline.capture.starts",
    ends: "pipeline.capture.ends",
  },
  {
    label: "pipeline.transcription",
    starts: "pipeline.transcription.starts",
    ends: "pipeline.transcription.ends",
  },
  {
    label: "pipeline.analysis",
    starts: "pipeline.analysis.starts",
    ends: "pipeline.analysis.ends",
  },
...
function dotClass(state: StepState): string {
  ...
  if (state === "active")
    return "animate-pulse bg-purple-500 border-purple-400";
      ...
      <div className="mb-1 flex items-center gap-2">
        {status !== "failed" ? (
          <>
            ...
            <p className="text-sm font-semibold">{t("pipeline.processing")}</p>
          </>
        ) : (
          ...
          return (
            ...
            >
              <div className="flex items-center md:w-full">
                <span
                  className={`h-3.5 w-3.5 shrink-0 rounded-full border-2 ${dotClass(state)}`}
                />
              ...
              <div className="min-w-0 md:pr-4">
                ...
                >
                  {t(step.label)}
                  {state === "queued" && t("pipeline.queuedSuffix")}
                </p>
                {step.starts && (
                  <p className="text-xs text-zinc-600">{t(step.starts)}</p>
                )}
                {step.ends && (
                  <p className="text-xs text-zinc-600">{t(step.ends)}</p>
                )}
                {isQueuedHere && queue && (
                  <p className="mt-1 text-xs text-amber-400">
                    {t("pipeline.position", { n: queue.position ?? "" })}
                    {queue.eta_seconds != null &&
                      t("pipeline.eta", {
                        n: Math.max(1, Math.round(queue.eta_seconds / 60)),
                      })}
                  </p>
                )}
              </div>
